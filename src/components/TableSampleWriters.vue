...
const getAllCategories = (page = 1) => {
  loading.value = true;
  axiosClient.get(`/v1/writers?page=${page}`).then((res) => {
    loading.value = false;
...
const deApprove = (id) => {
  ...
  confirmButtonText: 'Yes, de-approve it!'
}).then((result) => {
  if (result.isConfirmed) {
    axiosClient.post(`/v1/de-approve-writer/${id}`).then(() => {
      swal.fire(
        'De-Approve!',
        'Writer de-approved successfully.',
        'success'
      )
      setTimeout(() => {
        location.reload()
...
const approve = (id) => {
  ...
  if (result.isConfirmed) {
    axiosClient.post(`/v1/approve-writer/${id}`).then(() => {
      swal.fire(
        'Approve!',
...
<template>
  <CardBoxModal v-model="isModalActive" title="Show Writers">
    <div v-if="loading">
  ...
  <table>
    ...
    <tbody v-else>
      <tr v-for="client in allCategories.data" :key="client.id">
        ...
        <td data-label="Name">
          <div style="color: green" v-if="client.admin_verified == true">
            Verified
          </div>
