...
import TableSampleTag from "@/components/TableSampleTag.vue";
...
const submit = () => {
  loading.value = true;
  axiosClient.post('/v1/tags', form).then((res) => {
    loading.value = true;
...
<template>
  <LayoutAuthenticated>
    <SectionMain>
      ...
      <CardBox form @submit.prevent="submit">
        <FormField label="Grouped with icons">
          <FormControl v-model="form.name" type="text" placeholder="Tag name"/>
          <FormControl v-model="form.description" type="text" placeholder="Tag description"/>
        </FormField>
    ...
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiTableBorder" title="Tags table" main>
       
      </SectionTitleLineWithButton>
    

      <CardBox class="mb-6" has-table>
        <TableSampleTag checkable />
      </CardBox>
